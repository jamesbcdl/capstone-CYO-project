...
* The number of very high duration tickets is troublesome. If it is bad recordkeeping, that can be fixed. If we are mis-classifying tickets (for example, using them from everything from major projects to a note to call someone back), then they should not be ente3red into Cherwell. For example, updating 450 desktops to a new virus software package is *not* a ticket, it is a project. We would expect projects to take weeks or months to finish.
